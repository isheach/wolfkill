...
$env:DOTNET_ROOT='C:\Users\UIC\.dotnet'; $env:PATH="$env:DOTNET_ROOT;"+$env:PATH
dotnet run
# 期望: 238 通过, 0 失败(覆盖建模/存档/战斗状态机/状态/即死/穿透/召唤/灵脉/圣杯/礼装次数/世界运营/多单位槽位与等级补正/大航海结算等)
```
...
- 引擎测试覆盖: 槽位模板 7 档、容量移出、仆役位、支援位属性、等级补正与主力替补(engine-test 238/238)。

## 大航海战斗表结算(可切换, 2026-09 新增)

《大航海战斗表》是另一套基于男爵圣杯规则的战斗结算表(对方 GM 的表格, 文件 `../大航海战斗表.xlsx`)。
本版按其实测公式移植为引擎模块 `scripts/Engine/SeaSettle.cs`, 游戏内两条入口:

- **主菜单 → 大航海战斗表(结算流程)**: GM 计算器 + 流程展示。可选双方(1v1)/三方混战, 每方填
  主力+辅助×4、魔力消耗与礼装/令咒补充、胜率补正各项、手动优劣调整; 结算后按
  ①初始工序 ②主要工序 ③最终工序 输出工序表格(与 Excel 表段一一对应)。
- **对战页 → 初始工序勾选「大航海战斗表结算」**: 实战对局改用同一套结算链
  (对抗属性取 左方主要属性 / 右方主要属性 / 随机属性; 联机时为本地开关, 以主机为准)。

| 工序 | 规则 | 表内公式 |
|---|---|---|
| ①初始 | 最终魔力 = 当前魔力 + 礼装补充 + 令咒补充 − 消耗 | `U14 = U6 - SUM(U7:U13)` |
| ①初始 | 魔力不足: 每 −20 → 除宝具外全属性 −10; 单独行动减半 | `U15 = INT(U6/-20)*10`, `U16 = U15/2` |
| ①初始 | 角色保底: 属性不低于 0 | `MAX(0, …)` |
| ②主要 | 属性总值 = 主力全额 + Σ辅助÷2; 等级总值只算主力 | `H44/H45` |
| ②主要 | 战前/胜率补正: 主力全额、辅助减半(可关闭); 惩罚全额求和 | `H26/H27`, `H51/H52` |
| ②主要 | 战斗属性 = 属性总值 + 手动优劣×10 + 补正一(宝具除外) + 补正二 | `D58` |
| ③最终 | 基础胜率取优劣组合表: 3优90 / 2优1平80 / 2优1劣70 / 1优2平60 / 3平50 / 1优1平1劣50 / 2平1劣40 / 1优2劣30 / 1平2劣20 / 3劣10 | `E98` |
| ③最终 | 胜率合计 = 主力等级 + 基础胜率 + 战斗属性总值 + 战前 + 初始 + 主要 + 补充×2 | `L98 = SUM(D98:K98)` |
| ③最终 | 最终胜率 = clamp(50 + 胜率差值÷2, 底限, 100−对方底限); 另给出差值减半版 | `O98`, `Q98` |
| 三方 | 四维优劣: 单属性 一优二劣30 / 二平一劣15 / 三平10, 四属性求和再 +10 | `C101` |
| 三方 | 等差(等级差加乘, 「星之开拓者」时归零) + 战力差加乘(对三方均值) | `B101`, `D101` |
| 三方 | 保有胜率 150 分制: 另两方低于基准 → 150−两者基准; 否则按相对胜率比例分配 | `K101`, `E106/F106` |
| 三方 | 有负胜率时排除负者, 幸存方按相对胜率比例分配 150 分, 再归一为百分比 | 表内 104:108 备用表 |

自测用表内实例校验: 蓝方(醉枭赌徒 主力 + 焰狼/哈基米嗷 辅助) vs 橙方(机甲少女) →
属性总值 95/145/220、战斗属性 460:210、基础胜率 90:10、胜率合计 640:265、差值 375、最终胜率 100%:0%,
与 Excel 完全一致(engine-test 238/238)。
